...
export const createEvent = mutation({
  ...
  handler: async (ctx, args) => {
    try {
      ...
      // ✅ Get Pro status from DB (NEVER trust frontend)
      const hasPro = user.hasPro ?? false;

      // ✅ Free user event limit check
      ...
      // ✅ Update free event count (only for free users)
      if (!hasPro) {
...
export const deleteEvent = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Decrease free event count (only if free user)
    const hasPro = user.hasPro ?? false;
